...
var SUPPORTED_ANGULAR_VERSIONS = { min: 6, max: 9 };
function validateAngularVersion() {
    ...
    if (version < min || version > max) {
        throw new errors_1.BeagleCliError("You're using Angular " + version + ". Beagle offers support only for versions " + min + " to " + max);
    }
}
...
function getAngularJson(path) {
    try {
        ...
        throw new errors_1.BeagleCliError('Could not find your angular.json file!');
    }
}
