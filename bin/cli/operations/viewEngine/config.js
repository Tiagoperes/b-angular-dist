...
function fixConfigurationPath(path, root) {
    return path && root + "/" + path.replace(/^\.\//, '');
}
function fixConfigurationPaths(config) {
    var root = process.cwd();
    return tslib_1.__assign(tslib_1.__assign({}, config), { beagleModulePath: fixConfigurationPath(config.beagleModulePath, root), outputPath: fixConfigurationPath(config.outputPath, root) });
}
